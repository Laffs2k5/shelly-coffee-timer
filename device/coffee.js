...
// v2: local-MQTT transport + connectivity watchdog + state-resume across reboot.
// See docs/spec/11-local-mqtt.md (design) and 12-watchdog-validation.md (reset_reason).
//
// ⚠️ DEPLOY THE MINIFIED BUILD, NOT THIS FILE. The mJS heap is tiny: this commented source
// (~15 KB) crashes the device with out_of_memory. Flash device/coffee.min.js (generated by
// scripts/build-device.sh). Verified on hardware — see docs/testing/HW-VALIDATION-v2.md.
//
// MQTT transport (broker, mTLS, topic_prefix) is configured on the device via RPC
// (Mqtt.SetConfig + cert upload) — NOT here. This script only pub/subs topic names.

// --- Identity & topics (replaces the Adafruit feeds) ---
...
// Watchdog thresholds (see spec 11 §6.1). Gated on reset_reason resume (§6.3, validated).
...
// KVS-persisted runtime timer state (for resume across watchdog/software reboot)
let boot_rt_on = 0;
...
// Watchdog counters (seconds of continuous down-time)
let wifi_down_sec = 0;
let mqtt_down_sec = 0;
...
// NOTE: mJS JSON.parse() returns undefined on malformed input (it does NOT throw),
// so callers just check `typeof x !== "object"`. We deliberately avoid try/catch
// (uncertain in mJS). The Node test harness injects a JSON.parse with the same
// undefined-on-failure semantics so tests match device behaviour.

// Persist runtime timer state so a watchdog/software reboot can resume it.
// One KVS key (compact JSON) instead of three — fewer keys, and ONE write per persist
// (lower peak allocation; the original OOM was a write burst). Format: {on,r,m}.
function persist_rt() {
...
// Heartbeat publish with 2-second debounce. retain=true (the app's "last seen").
...
// Infra liveness (spec 11 §4.1). Non-retained, payload irrelevant, NTP-independent.
...
// MQTT config handler. config is retained -> delivered automatically on (re)subscribe.
...
// MQTT connect handler. No /get needed — config is retained.
function on_mqtt_connect() {
  do_publish_heartbeat(true);
}

// Main loop: single 30-second repeating timer handles everything
// - alive publish (every 60s, NTP-independent)
// - Tick countdown (every 60s) + periodic rt_remain persist while ON
...
function main_loop() {
  // 60-second cadence block
  tick_counter = tick_counter + 1;
  if (tick_counter >= 2) {
    tick_counter = 0;
    // Infra liveness — always, regardless of NTP or on/off state.
    publish_alive();
    // Countdown
    if (sw_on) {
      remain = remain - 60;
  ...
  // Connectivity watchdog (spec 11 §6.1). Gated implicitly on the resume gate:
  // a reboot resumes the timer only when reset_reason==3, which Shelly.Reboot produces.
...
// Boot complete: decide resume-vs-OFF, then set up everything.
function boot_complete() {
  // --- Resume gate (spec 11 §6.3, validated in doc 12) ---
  // Resume the persisted countdown ONLY on a software/watchdog reboot (reset_reason==3).
  // Power loss (1) and anything else boot OFF (safety invariant preserved).
  let resume = false;
  ...
  // MQTT subscriptions (config arrives retained on subscribe)
  ...
  Shelly.addStatusHandler(function(event) {
    ...
    // the switch directly in firmware. We use script_switching to distinguish
    // our Switch.Set calls from physical button presses.
