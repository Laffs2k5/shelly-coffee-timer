# Architecture Overview (Adafruit IO — v1.x, DEPRECATED)

> **⚠️ This document describes the v1.x Adafruit IO architecture, which is being deprecated and replaced.**
>
> The Adafruit IO cloud-broker design (REST + cloud MQTT) is the **v1.x** generation of
> this project. It is frozen at the [`adafruit-final`](https://github.com/Laffs2k5/shelly-coffee-timer/releases/tag/adafruit-final)
> tag. A re-architecture to **local MQTT with a bridge to a new cloud broker** is in
> progress and will replace this design entirely (rip-and-replace).
>
> This file is an archived historical snapshot. For the current/target architecture, see
